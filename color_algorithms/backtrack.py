...
from tests.test_backtrack import test_backtrack

def main(backtrack_queue, sudoku_dataset):
  """
    This function have the logic to create the 
  tests of the colouring backtrack algorithm.
  """
  number_of_next_row = 0
  tests_finished = mp.Queue()
  tests_running = mp.Queue()
  process_list = []
  while tests_finished.qsize() < 100: # While don't exists 100 test finished run more tests
    if tests_running.qsize() < 100: # Must have 10 tests running at the same time
      ...
      process = mp.Process(target=test_backtrack, args=(backtrack_queue, graph, tests_finished, tests_running))
