...
from tests.test_ldo import test_ldo

def main(ldo_queue, sudoku_dataset):
  """
  ...
  while tests_finished.qsize() < 100: # While don't exists 100 test finished run more tests
    if tests_running.qsize() < 10: # Must have 10 tests running at the same time
      ...
      process = mp.Process(target=test_ldo, args=(ldo_queue, graph, tests_finished, tests_running))
